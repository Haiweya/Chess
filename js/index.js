...
var block = false;//单个落棋点点击锁
var start_block=false; //总开关

var btnpeople= document.getElementById('btnpeople');
var btncomputer= document.getElementById('btncomputer');

// var loadAiBlock = true;
function createGrid(x, y) {//创建棋盘节点div
    var temp = document.createElement("div");
    temp.classList.add("grid");
    temp.style.left = (18 + x * 30 )+ "px";
    temp.style.top = (18 + y * 30) + "px";
    temp.x = x;
    temp.y = y;
...
function createFont(){
    ...
    font.innerHTML='start game';
    startBox.classList.add('start');
    wrapper.appendChild(startBox);
    startBox.appendChild(font);


    setTimeout(function(){
        startBox.removeChild(font);
        wrapper.removeChild(startBox);
    },2000)
}
...
function init() {//初始化方法
    ...
    var btn_blockcum=true; 
    var btn_blockpeo=true; 
      btncomputer.onclick=function(){
            if(btn_blockcum){
                btn_blockpeo=false;
                btn_blockcum=false;
                AI=true;
               ...
               if(start_block){
                    ...
                    if (result == 0) {//没结束
                        ...
                        if (AI && count % 2 == 1) {//判断是否有AI
                            ...
                            // newscript.setAttribute('src','./js/AI.js');
